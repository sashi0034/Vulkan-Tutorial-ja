...
Vulkan が物理シミュレーションをどのように高速化できるかを検討する前に、エンジンに基本的な物理システムを実装して基礎を確立しましょう。これにより、後で Vulkan コンピューティング機能で強化できるフレームワークが得られます。

=== 物理システムのアーキテクチャ

典型的なゲーム物理システムは、いくつかの主要なコンポーネントで構成されます。

* *剛体ダイナミクス*: 質量、速度、回転特性を持つ固体オブジェクトのシミュレーション。
* *衝突検出*: オブジェクトがいつ交差または相互に接触したかを判断します。
* *衝突応答*: オブジェクトが衝突したときにオブジェクトがどのように反応するかを計算します。
* *制約*: ジョイント、ヒンジ、またはその他の接続に基づいてオブジェクトの動きを制限します。
* *継続的衝突検出*: フレーム間で他の物体を通過する可能性のある高速移動オブジェクトを処理します。
* *空間分割*: 世界を領域に分割することで衝突検出を最適化します。

エンジンの設計と一致する最新の C++ アプローチを使用して、これらの基本をカバーする単純な物理システムを実装してみましょう。

=== 基本的な物理システムの実装

まず、物理システムのコア クラスを定義します。
...
この基本構造は、衝突の検出と応答を備えた剛体の物理学をシミュレートするための基礎を提供します。実際の実装では、より高度な機能や最適化のために、Bullet、PhysX、Havok などの物理ライブラリを使用することになるでしょう。

=== エンジンとの統合

物理システムをエンジンの残りの部分と統合するには、それをエンジンのメイン クラスに追加します。
...
更新ループをわかりやすくするために、固定タイムステップ フレームを 6 つのステップとして考えます。

1) 力を蓄積する (例: 重力、ユーザーの力)
2) 力を統合します (減衰を使用して速度を更新します)
3) 衝突の検出 (ペアごとに広い/狭いチェック)
4) 衝突の解決 (インパルス + 位置補正)
5) 速度の積分 (位置と方向の更新)
6) 戦力のクリア（次のステップの準備）

コアの物理シミュレーション関数を実装しましょう。
...
この基本的な物理システムは、ゲーム内で剛体をシミュレートするために不可欠な機能を提供しますが、いくつかの制限があります。

1. *パフォーマンス*: O(n²) 衝突検出は、多くのオブジェクトではボトルネックになります。
2. *制限された衝突形状*: ボックスや球などの基本的な形状のみを実装しました。
3. *安定性の問題*: 単純なインテグレータと衝突解決は不安定性を引き起こす可能性があります。
4. *連続衝突検出なし*: 高速で移動する物体が薄い障害物を通り抜ける可能性があります。
5. *制限された制約*: ジョイント、スプリング、その他の制約は実装されていません。
6. *CPU バウンド処理*: すべての計算は CPU 上で実行されるため、スケーラビリティが制限されます。

次のセクションでは、Vulkan コンピューティング シェーダーが、特に多数のオブジェクトを含む大規模シミュレーションの場合に、物理計算を GPU にオフロードすることでこれらの制限にどのように対処できるかを検討します。

xref:Building_a_Simple_Engine/Subsystems/03_vulkan_audio.adoc[前: オーディオ処理用の Vulkan] | xref:Building_a_Simple_Engine/Subsystems/05_vulkan_physics.adoc[次へ: 物理シミュレーション用の Vulkan]
