:pp: {plus}{plus}
= デバッグドロワー: 物理とスケルトンの可視化

== デバッグ可視化の原則

デバッグ描画コードを書く前に、設計原則を理解しておく価値があります。デバッグドロワーはレンダリングパイプラインの恒久的な一部ではありません。リリースビルドではコンパイル時に消え、デバッグまたは開発ビルドで有効になるモード切替オーバーレイです。次の性質を持つべきです。

- **単純。** デバッグ幾何は最小限のパイプラインで描画—ライティング、テクスチャ、シャドウなし。単色またはワイヤーフレームの色付き線と形状。
- **常に手前。** デバッグ幾何は通常、深度テストを無効（または非常に大きな深度バイアス）にして、不透明な幾何に隠れないようにします。衝突カプセルがメッシュの内側にあっても見える必要があります。
- **非アクティブ時はオーバーヘッドゼロ。** リリースビルドではすべてのデバッグ描画呼び出しは何も生成しないようにします。プリプロセッサフラグまたはコンパイル時定数で完全に除去します。

実装パターンは **遅延ラインバッファ** です。各デバッグ形状ごとに即座にドローコールを発行するのではなく、フレームのすべての線分を CPU 側バッファに蓄積し、フレーム末に一度だけバッファをアップロードし、単純な非ライティングシェーダーで 1 回のインデックスドローコールで描画します。高速で単純、GPU にも優しいです。
...
色付き線の描画には専用のグラフィックスパイプラインが必要です。壁越しにもデバッグ幾何を見せたいため、深度テストを無効にするか、非常に大きな深度バイアスを使うのが一般的です。デバッグラインパイプラインの完全な設定は次のとおりです。
...
スケルトンは最も重要なデバッグオーバーレイです。シーングラフのトランスフォーム階層を可視化し、各ジョイントのワールド空間上の位置と、どのジョイントがどのボーンの親かを正確に示します。
...
衝突形状の描画では、物理エンジンから各ボディの現在のトランスフォームを問い合わせ、格納された `ColliderDef` で形状パラメータを決めます。
...
物理拘束については、`PhysicsWorld` インターフェースは汎用の拘束問い合わせ API を公開しません。Jolt では拘束のイントロスペクションはエンジン固有だからです。以下のコードは **疑似コード** で、拘束ハンドルを具象 Jolt 型（`JPH::Constraint*`）にキャストし、`mPoint1`、`mHingeAxis1` などを直接読むパターンを示します。
...
第 4 章のラグドール引き渡しが失敗してボディが飛び散るとき、これらのオーバーレイですぐに原因が分かります。拘束が保持に失敗しているのか（カプセルが接触しなくなり、接続線が非常に長い）、ボディの初期位置が誤っているのか（カプセルが描画メッシュからずれている）。

xref:Advanced_glTF/Debugging_Visual_Auditing/01_introduction.adoc[前へ: はじめに] | xref:Advanced_glTF/Debugging_Visual_Auditing/03_skinning_heatmaps.adoc[次へ: スキニングヒートマップ]
