...
= Khronos Vulkan^®^ Tutorial（日本語版）

このリポジトリは link:https://docs.vulkan.org/tutorial/latest[Khronos Vulkan Tutorial] の日本語訳フォークです。
英語原文は `en/`、日本語訳は `ja/` に並存します。Antora のビルド対象は `ja/` です。
用語の統一については link:TRANSLATION_GLOSSARY.md[TRANSLATION_GLOSSARY.md] を参照してください。
...
This repository hosts the contents of the link:https://docs.vulkan.org/tutorial/latest[Khronos Vulkan Tutorial] (Japanese translation). The upstream tutorial is part of the link:https://github.com/KhronosGroup/Vulkan-Site[Vulkan Documentation Project].
...
* `antora/` - Contains configuration files for the Antora documentation system（pages は `ja/` から同期）
* `TRANSLATION_GLOSSARY.md` - 日本語訳の用語集
